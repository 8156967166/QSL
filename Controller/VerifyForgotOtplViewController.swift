//
//  VerifyForgotOtplViewController.swift
//  PrjctQsl
//
...
import SJFrameSwift
    ...
    class VerifyEmailOtpViewController: SJViewController, UITextFieldDelegate {
        ...
        @IBOutlet weak var buttonResendTimer: UIButton!
        var getEmail = String()
        ...
        override func viewDidLoad() {
            ...
            buttonResendTimer.isEnabled = false
            self.viewNotEnterOtp.isHidden = true
    ...
    //        self.labelPleaseEnter.text = "Please Enter The Code Sent To The Email \(getEmail)"
            let mainString = "Please Enter The Code Sent To The Email \(getEmail)"
            let stringToColor = "\(getEmail)"
    ...
    //        }
        ...
        @objc func timerAction() {
            ...
            if counter == 0 {
                buttonResendTimer.isEnabled = true
...
//        }
        @IBAction func buttonActionChangeEmail(sender: UIButton) {
            self.performSegue(withIdentifier: "UpdateEmailViewController", sender: nil)
        }
    ...
    extension VerifyEmailOtpViewController {
        func callForgotPasswordOtpApi() {
            if AppController.shared.checkInternetAvailability() {
                ...
                objRequest.rawParam = ["user_name":"\(getEmail)", "otp": "\(textFieldFirstNumberOfOtp.text ?? "")\(textFieldSecondNumberOfOtp.text ?? "")\(textFieldThirdNumberOfOtp.text ?? "")\(textFieldFourthNumberOfOtp.text ?? "")\(textFieldFifthNumberOfOtp.text ?? "")", "purpose_of_request": "activate_user"].toJSON()
                self.showLoader()
